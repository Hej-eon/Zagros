...
function drawZagrosHero(x,y){const X=Math.round(x*TILE),Y=Math.round(y*TILE);ctx.imageSmoothingEnabled=false;const r=(color,a,b,w,h)=>{ctx.fillStyle=color;ctx.fillRect(X+a,Y+b,w,h)};r('#5a351f',14,38,9,5);r('#5a351f',27,38,9,5);r('#242b2d',15,29,19,10);r('#182025',18,31,5,7);r('#182025',27,31,5,7);r('#d8a32d',21,28,7,7);r('#b43a3a',12,24,11,11);r('#b43a3a',25,24,11,11);r('#fff0cf',14,22,8,7);r('#fff0cf',26,22,8,7);r('#f0d5a0',16,13,17,12);r('#3a2525',18,18,3,3);r('#3a2525',28,18,3,3);r('#fff0cf',15,11,19,4);r('#b43a3a',12,9,24,6);r('#f3d35b',15,7,19,3);r('#fff0cf',18,8,12,3);r('#b43a3a',12,12,4,7);r('#b43a3a',32,12,4,7);r('#e8b83f',21,24,6,5)} function draw(){ctx.clearRect(0,0,COLS*TILE,ROWS*TILE);for(let y=0;y<ROWS;y++)for(let x=0;x<COLS;x++){ctx.fillStyle=(x+y)%2?'#20352d':'#1b3029';ctx.fillRect(x*TILE,y*TILE,TILE,TILE);if(board[y][x].solid){const X=x*TILE,Y=y*TILE;ctx.fillStyle='#34443a';ctx.fillRect(X+1,Y+1,TILE-2,TILE-2);ctx.fillStyle='#4e5d4d';ctx.fillRect(X+4,Y+5,TILE-8,13);ctx.fillRect(X+4,Y+21,TILE-18,13);ctx.fillRect(X+25,Y+21,TILE-8,13);ctx.fillStyle='#69745d';ctx.fillRect(X+5,Y+5,TILE-14,5);ctx.fillRect(X+5,Y+21,TILE-25,4);ctx.fillRect(X+27,Y+21,TILE-11,4);ctx.fillStyle='#26342d';ctx.fillRect(X+4,Y+34,TILE-8,5);ctx.fillRect(X+18,Y+18,6,4);ctx.fillRect(X+35,Y+5,4,10);}else if(board[y][x].brick){const X=x*TILE,Y=y*TILE;ctx.fillStyle='#3b2417';ctx.fillRect(X+2,Y+2,TILE-4,TILE-4);ctx.fillStyle='#754522';ctx.fillRect(X+5,Y+5,TILE-10,TILE-10);ctx.fillStyle='#a96532';ctx.fillRect(X+7,Y+7,TILE-14,6);ctx.fillStyle='#8a5329';ctx.fillRect(X+7,Y+14,TILE-14,20);ctx.fillStyle='#c27a38';ctx.fillRect(X+7,Y+34,TILE-14,4);ctx.fillStyle='#4b2d1a';ctx.fillRect(X+8,Y+12,TILE-7,3);ctx.fillRect(X+8,Y+31,TILE-7,3);ctx.fillRect(X+8,Y+15,4,16);ctx.fillRect(X+36,Y+15,4,16);ctx.fillStyle='#d18a43';ctx.fillRect(X+9,Y+8,TILE-20,3);ctx.fillStyle='#241b16';for(const p of [[8,8],[40,8],[8,37],[40,37]])ctx.fillRect(X+p[0],Y+p[1],3,3);ctx.strokeStyle='#b56d34';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(X+13,Y+18);ctx.lineTo(X+35,Y+30);ctx.moveTo(X+35,Y+18);ctx.lineTo(X+13,Y+30);ctx.stroke();}}for(const p of powerups){ctx.fillStyle=p.type==='range'?'#2ca58d':'#e4b43c';ctx.beginPath();ctx.arc(p.x*TILE+24,p.y*TILE+24,10,0,Math.PI*2);ctx.fill();ctx.fillStyle='#07111f';ctx.font='bold 15px sans-serif';ctx.textAlign='center';ctx.fillText(p.type==='range'?'R':'B',p.x*TILE+24,p.y*TILE+29)}for(const b of bombs){const X=b.x*TILE,Y=b.y*TILE;ctx.fillStyle='#0f1215';ctx.beginPath();ctx.arc(X+24,Y+27,14,0,Math.PI*2);ctx.fill();ctx.fillStyle='#2d3338';ctx.fillRect(X+16,Y+17,10,5);ctx.fillStyle='#59616a';ctx.fillRect(X+18,Y+15,6,3);ctx.strokeStyle='#4b3524';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(X+31,Y+16);ctx.quadraticCurveTo(X+35,Y+11,X+39,Y+8);ctx.stroke();ctx.fillStyle='#f4b52f';ctx.fillRect(X+38,Y+5,4,4);ctx.fillStyle='#f8d45a';ctx.fillRect(X+40,Y+3,2,2);}for(const f of flames){ctx.fillStyle='#d96b27';for(const[cx,cy]of f.cells){ctx.fillRect(cx*TILE+7,cy*TILE+7,TILE-14,TILE-14);ctx.fillStyle='#f0c04a';ctx.fillRect(cx*TILE+15,cy*TILE+15,TILE-30,TILE-30);ctx.fillStyle='#d96b27'}}for(const e of enemies){ctx.fillStyle='#8d2f36';ctx.beginPath();ctx.arc(e.px*TILE+24,e.py*TILE+25,15,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff';ctx.fillRect(e.px*TILE+15,e.py*TILE+19,5,5);ctx.fillRect(e.px*TILE+28,e.py*TILE+19,5,5)}if(player.alive){drawZagrosHero(player.px,player.py)}}
